...
| Image | Download → save to `.vibemote/media/` → reference path in prompt → Claude reads natively |
| Document | Download → save to `.vibemote/media/` → reference path in prompt |
...
- PID file at `~/.vibemote/daemon.pid` prevents duplicates
- Last session IDs persisted at `~/.vibemote/state.json` for crash recovery

### CLI Commands

| Command | Action |
|---|---|
| `vibemote init` | First-time setup (user ID, whisper model download) |
| `vibemote add <path>` | Register project (BotFather walkthrough) |
| `vibemote remove <name>` | Unregister project |
| `vibemote list` | Show all registered projects |
| `vibemote start` | Start daemon (all bots) |
| `vibemote stop` | Stop daemon |
| `vibemote restart` | Stop + start |
| `vibemote status` | Show bot statuses, uptime |
| `vibemote logs [name]` | Tail logs (all or specific project) |
| `vibemote install` | Create macOS LaunchAgent (auto-start on login) |
| `vibemote uninstall` | Remove LaunchAgent |
| `vibemote doctor` | Health check (node, API key, whisper model) |

### macOS LaunchAgent

Generated by `vibemote install`:
- `~/Library/LaunchAgents/com.vibemote.daemon.plist`
- `RunAtLoad: true` — starts on login
- `KeepAlive: true` — restarts on crash
- Logs to `~/.vibemote/logs/`
...
### Config File: `~/.vibemote/config.json`
